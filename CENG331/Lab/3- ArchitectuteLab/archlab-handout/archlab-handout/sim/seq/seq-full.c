...
long long gen_need_regids()
{
    return ((icode) == (I_RRMOVQ) || (icode) == (I_ALU) || (icode) == 
      (I_PUSHQ) || (icode) == (I_POPQ) || (icode) == (I_IRMOVQ) || (icode)
       == (I_RMMOVQ) || (icode) == (I_MRMOVQ) || (icode) == (I_ISUBQ));
}
...
long long gen_srcB()
{
    return (((icode) == (I_ALU) || (icode) == (I_RMMOVQ) || (icode) == 
        (I_MRMOVQ) || (icode) == (I_ISUBQ)) ? (rb) : ((icode) == (I_PUSHQ)
...
long long gen_dstE()
{
    return ((((icode) == (I_RRMOVQ)) & (cond)) ? (rb) : ((icode) == 
        (I_IRMOVQ) || (icode) == (I_ALU) || (icode) == (I_ISUBQ)) ? (rb) : 
...
long long gen_aluA()
{
    return (((icode) == (I_RRMOVQ) || (icode) == (I_ALU)) ? (vala) : (
        ...
        (I_MRMOVQ)) ? (valc) : ((icode) == (I_ISUBQ)) ? (valb) : ((icode)
...
long long gen_aluB()
{
    return (((icode) == (I_RMMOVQ) || (icode) == (I_MRMOVQ) || (icode) == 
        ...
        (I_ISUBQ)) ? (valc) : ((icode) == (I_RRMOVQ) || (icode) == 
        (I_IRMOVQ)) ? 0 : 0);
}
...
{
    return ((icode) == (I_ALU) || (icode) == (I_ISUBQ));
}
