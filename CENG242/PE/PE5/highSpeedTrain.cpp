...
std::string operator+(const HighSpeedTrain &h1, const HighSpeedTrain &h2)
{
    ...
    if (h1.to == h2.from)
    {
        int eta = (h1.eta + h2.eta);
        return h1.from + " -> " + h1.to + " -> " + h2.to + " in " + std::to_string(eta) + " mins.";
    }
...
{
    // YOUR CODE HERE
    if(h1.from < h2.from){return true;}
    else if((h1.from == h2.from) && (h1.to < h2.to)){return true;}
    else if((h1.from == h2.from) && (h1.to == h2.to) && (h1.eta < h2.eta)){return true;}
    else{return false;}
}
